...
if ($dbConn = true) {
	...
	if (isset($_GET['sbmt'])) {
		...
		if (strlen($tableName) > 20) {
			...
			$deleteComTable ="DROP TABLE IF EXISTS ".$tableName."";
			
			$query_one = mysqli_query($connection, $deleteComTable);
...
<html>
<head>
	<title>Delete Company tables</title>
	<style type="text/css">
		body{
...
<body>
	<div id="formDiv">
		<form action="" method="GET">
			...
			<input type="submit" name="sbmt" value="delete company">
		</form>
	</div>
	<div id="selectionDiv">
		<?php
			$selection = "SHOW TABLES FROM alphacrm LIKE '%comp%'";
			...
			$result = mysqli_query($connection, $selection);
				...
				if ($result) {
					...
					echo "<h4>Current Companies</h4><br>";
